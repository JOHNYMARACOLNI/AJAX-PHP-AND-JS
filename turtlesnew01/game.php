...
<body>
    ...
    function updateTurtlePosition(playerId, newField, color) {
    const turtle = document.getElementById(`turtle-${playerId}`);
    if (!turtle || newField === 0) return; // 0 = pozycja startowa

    // TO JEST GLUPOTA ZROB DRUGIEGO FETCHA DO POZYCJI POL
    const fieldData = spriteData.board.fields.find(f => f.position === newField);
    if (!fieldData) return;
    ...
    gsap.to(turtle, {
        ...
        ease: "power2.out",
        onComplete: () => {
            // Ewentualne dodatkowe efekty po ruchu
            if (newField === 10) {
                // Efekt zwycięstwa jeśli dotarł do końca
                gsap.to(turtle, {
                    scale: 1.3,
                    yoyo: true,
                    repeat: 1,
